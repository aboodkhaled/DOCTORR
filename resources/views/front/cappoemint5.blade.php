 ...
 <div class="page-banner overlay-dark bg-image" style="background-image: url({{asset('assets/front/u/assets/img/bg_image_1.jpg')}});">
  ...
  @if ($errors->any())
    ...
    @foreach($clinic as $_clinic)
      <h1 class="text-center wow fadeInUp">حجز خدمة     <br> في عيادة / {{$_clinic -> name}} </h1>
      ...
      <form class="form col-md-12 " action="{{route('cappoemintt5s.save')}}" method="POST"
          ...
          <div class="col-12 col-sm-8 py-2 text-center">
            <div class="form-group">
              <label for="projectinput2"><h3>{{trans('servee_trans.Choose serve name')}}</h3>   </label>
                 <select style="height:48px;" name="serve5_id" id="serve5_id" class="select form-control"
                               ...
                               value="{{$serve1 -> id }}">    {{$serve1 -> name}} 
                                                    ...
                                                    <div class="col-12 col-sm-12 py-2 text-center">
                                                        <div class="form-group">
                                                            ...
                                                            <select  style="height:48px;" name="serve5_thin_id" id="serve5_thin_id" class="select form-control">
                                                    ...
                                                    <div class="col-12 col-sm-4 py-2 text-center">
                                                        <div class="form-group">
                                                            ...
                                                            <select  style="height:48px;" name="serve5_price_id" id="serve5_price_id" class="select form-control">
                                                               

                                                               
                                                            </select>
                                                            @error('serve5_price_id')
                                                            <span class="text-danger"> {{$message}}</span>
                                                            @enderror
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-12 col-sm-4 py-2 text-center">
                                                        <div class="form-group">
                                                            ...
                                                            <select  style="height:48px;" name="serve5_tprice_id" id="serve5_tprice_id" class="select form-control">
                                                               

                                                               
                                                            </select>
                                                            @error('serve5_tprice_id')
                                                            <span class="text-danger"> {{$message}}</span>
                                                            @enderror
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-12 col-sm-4 py-2 text-center">
                                                        <div class="form-group">
                                                            ...
                                                            <select  style="height:48px;" name="serve5_total_id" id="serve5_total_id" class="select form-control">
                                                               

                                                               
                                                            </select>
                                                            @error('serve5_total_id')
                                                            <span class="text-danger"> {{$message}}</span>
                                                            @enderror
                                                        </div>
                                                    </div>
...
<script>
        $(function(){
            $(document).on('change','#serve5_id', function(){
                populateServe5_thin();
             return false;
            });
            function  populateServe5_thin(){
                $('option', $('#serve5_thin_id')).remove();
                $('#serve5_thin_id').append($('<option></option>').val('').html());
                var serveId = $('#serve5_id').val() != null ? $('#serve5_id').val() : '{{old('serve5_id')}}';
                $.get("{{Route('gethpric5e')}}",{serve5_id: serveId }, function(data){
               $.each(data, function(val, text){
                   var selectedVal = val == '{{old('serve5_thin_id')}}' ? "selected" : "";
                   $('#serve5_thin_id').append($('<option class="text-center  "'+selectedVal+'></option>').val(val).html(text));
               })
                }, "json");
            }
        });
    ...
    <script>
        $(function(){
            $(document).on('change','#serve5_id', function(){
                populateServe5_price();
             return false;
            });
            function  populateServe5_price(){
                $('option', $('#serve5_price_id')).remove();
                $('#serve5_price_id').append($('<option></option>').val('').html());
                var serveId = $('#serve5_id').val() != null ? $('#serve5_id').val() : '{{old('serve5_id')}}';
                $.get("{{Route('getppric5e')}}",{serve5_id: serveId }, function(data){
               $.each(data, function(val, text){
                   var selectedVal = val == '{{old('serve5_price_id')}}' ? "selected" : "";
                   $('#serve5_price_id').append($('<option '+selectedVal+'></option>').val(val).html(text));
               })
                }, "json");
            }
        });
...
<script>
        $(function(){
            $(document).on('change','#serve5_id', function(){
                populateServe5_tprice();
             return false;
            });
            function  populateServe5_tprice(){
                $('option', $('#serve5_tprice_id')).remove();
                $('#serve5_tprice_id').append($('<option></option>').val('').html());
                var serveId = $('#serve5_id').val() != null ? $('#serve5_id').val() : '{{old('serve5_id')}}';
                $.get("{{Route('gettpric5e')}}",{serve5_id: serveId }, function(data){
               $.each(data, function(val, text){
                   var selectedVal = val == '{{old('serve5_tprice_id')}}' ? "selected" : "";
                   $('#serve5_tprice_id').append($('<option '+selectedVal+'></option>').val(val).html(text));
               })
                }, "json");
            }
        });
...
<script>
        $(function(){
            $(document).on('change','#serve5_id', function(){
                populateServe5_total();
             return false;
            });
            function  populateServe5_total(){
                $('option', $('#serve5_total_id')).remove();
                $('#serve5_total_id').append($('<option></option>').val('').html());
                var serveId = $('#serve5_id').val() != null ? $('#serve5_id').val() : '{{old('serve5_id')}}';
                $.get("{{Route('getopric5e')}}",{serve5_id: serveId }, function(data){
               $.each(data, function(val, text){
                   var selectedVal = val == '{{old('serve5_total_id')}}' ? "selected" : "";
                   $('#serve5_total_id').append($('<option '+selectedVal+'></option>').val(val).html(text));
               })
                }, "json");
            }
        });
