...
class Appoemint4Controller extends Controller
{

    use MeetingZoomTrait;
    public function index(){
        $clinic = clinic::find(auth('clinic')->user()->id);
        $appoemint2s = appoemint4::where('clinic_id',$clinic -> id)->paginate(PAGINATION_COUNT);
        return view('clinic.appoemint4.index', compact('clinic','appoemint2s'));
    }
    public function create(){
       

      $clinic = clinic::find(auth('clinic')->user()->id);
      $serve1s = serve4::where('clinic_id',$clinic -> id)->get();
      $serve1_prices = serve4_price::where('clinic_id',$clinic -> id)->get();
      $serve1_thins = serve4_thin::where('clinic_id',$clinic -> id)->get();
      $users = User::get();
      $serve1_totals = serve4_total::where('clinic_id',$clinic -> id)->get();
      $serve1_tprices = serve4_tprice::where('clinic_id',$clinic -> id)->get();
        return view('clinic.appoemint4.create',compact('clinic','serve1s','serve1_prices','serve1_thins','users','serve1_totals','serve1_tprices'));
    }
    public function save(Request $request){
               ...
               $happoemints->save();
        return redirect()->route('clinic.appoemint4s')->with(['success' => trans('messages.success')]);
    
    }

    public function edit($id){
        try{
        $happoemint = appoemint4::find($id);
        if(!$happoemint){
        return redirect()->route('clinic.appoemint4s')->with(['error'=>'هذا الطبيب غير موجود']);}
      ...
      $hserves = hserve::where('hosbital_id',$hosbital -> id)->get();
        return view('clinic.appoemint4.edit',compact('happoemint','hosbital','hdepartments','hspecialtys','hdoctors','users','hdoctor_serves','hserves'));
        }catch(\Exception $ex){
            return redirect()->route('clinic.appoemint4s')->with(['error'=>'هناك خطاء ما يرجى المحاولة فيما بعد']);   
        }
    ...
    public function update($appoemint_id, Request $request){
        ...
        $happoemint = appoemint4::find($appoemint_id);
        if(!$happoemint)
        return redirect()->route('clinic.appoemint4s')->with(['error'=>'هذا الطبيب غير موجود']);
        DB::beginTransaction();
        ...
        return redirect()->route('clinic.appoemint4s')->with(['success' => trans('messages.Update')]);
    ...
    public function destroy($id){
        try{
            $happoemint = appoemint4::find($id);
            if(!$happoemint){
                return redirect() -> route('clinic.appoemint4s',$id)-> with(['error'=>'هذة ألطبيب غير موجودة']);
            }
            $happoemint -> delete();
            return redirect()->route('clinic.appoemint4s')->with(['success'=>trans('messages.Delete')]);
               }catch(\Exception $ex){
                return redirect()->route('clinic.appoemint4s')->with(['error'=>'هناك خطاء ما يرجى المحاولة فيما بعد']);        
                }    
        ...
        public function showdetail($id){
            $happoemint = appoemint4::where('id',$id)->first();
            $user = hoperation::where('appoemint4_id',$id)->get();
            $hmates = hmate::where('happoemint_id',$id)->get();
            $hdoctors = hdoctor::get();
            return view('clinic.appoemint4.detail',compact('user','happoemint','hmates','hdoctors'));
        }
