 ...
 <div class="page-banner overlay-dark bg-image" style="background-image: url({{asset('assets/front/u/assets/img/bg_image_1.jpg')}});">
  ...
  @if ($errors->any())
    ...
    <div class="container">
      ...
      <form class="form col-md-12 " action="{{route('happoemint.save')}}" method="POST"
        ...
        <div class="col-4 col-sm-4   py-2 wow fadeInLeft text-right">
          ...
          <label for="projectinput2"><h3>ألقسم / {{$doctors ->hdepartment-> dept_name}}</h3></label>
            <input type="hidden" name="hdepartment_id" class="form-control" value="{{$doctors ->hdepartment-> id}}" > <h1></h1>
          </div>
          <div class="col-4 col-sm-4   py-2 wow fadeInLeft text-left">
          <label for="projectinput2"><h3>ألتخصص /  {{$doctors ->hspecialty-> special_name}}</h3></label>
            <input type="hidden" name="hspecialty_id" class="form-control" value="{{$doctors ->hspecialty-> id}}" > <h1></h1>
          </div>
          </div>
          <div class="row col-12 ">
          ...
          <label for="projectinput2"><h3>أسم ألطبيب / {{$doctors -> doc_name}}</h3></label> 
            ...
            <input type="hidden" name="hdoctor_id" value="{{$doctors -> id}}" class="form-control" placeholder="Full name" ><h1> </h1>
          </div>

          <div class="col-6 col-sm-6   py-2 wow fadeInLeft text-center">
          <label for="projectinput2"><h3>أسم ألمستشفئ / {{$doctors ->hosbital-> name}}</h3></label> 
            <input type="hidden" name="hosbital_id" value="{{$doctors ->hosbital-> id}}" class="form-control" placeholder="Full name" ><h1> </h1>
          </div>
          </div>
          ...
          <div class="col-12 col-sm-4 py-2 text-center">
            <div class="form-group">
              <label for="projectinput2"><h3>{{trans('servee_trans.Choose serve name')}}</h3>   </label>
                 <select style="height:48px;" name="hserve_id" id="hserve_id" class="select form-control"
                      ...
                      <option>...............</option>
                                                    ...
                                                    <div class="col-12 col-sm-4 py-2 text-center">
                                                        <div class="form-group">
                                                            ...
                                                            <select  style="height:48px;" name="hdoctor_serve_id" id="hdoctor_serve_id" class="select form-control">
    ...
    <script>
        $(function(){
            ...
            function  populateDoctor_serves(){
                ...
                $.get("{{Route('hgetprice')}}",{hserve_id: serveId }, function(data){
