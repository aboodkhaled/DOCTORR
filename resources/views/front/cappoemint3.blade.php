 ...
 <div class="page-banner overlay-dark bg-image" style="background-image: url({{asset('assets/front/u/assets/img/bg_image_1.jpg')}});">
  ...
  @if ($errors->any())
    ...
    @foreach($clinic as $_clinic)
      <h1 class="text-center wow fadeInUp">حجز خدمة مجارحة  <br> في عيادة / {{$_clinic -> name}} </h1>
      ...
      <form class="form col-md-12 " action="{{route('cappoemintt3s.save')}}" method="POST"
          ...
          <div class="col-12 col-sm-8 py-2 text-center">
            <div class="form-group">
              <label for="projectinput2"><h3>{{trans('servee_trans.Choose serve name')}}</h3>   </label>
                 <select style="height:48px;" name="serve3_id" id="serve3_id" class="select form-control"
                               ...
                               value="{{$serve1 -> id }}">    {{$serve1 -> name}} 
                                                    ...
                                                    <div class="col-12 col-sm-12 py-2 text-center">
                                                        <div class="form-group">
                                                            ...
                                                            <select  style="height:48px;" name="serve3_thin_id" id="serve3_thin_id" class="select form-control">
                                                               

                                                               
                                                            </select>
                                                            @error('serve3_thin_id')
                                                            <span class="text-danger"> {{$message}}</span>
                                                            @enderror
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-12 col-sm-4 py-2 text-center">
                                                        <div class="form-group">
                                                            ...
                                                            <select  style="height:48px;" name="serve3_price_id" id="serve3_price_id" class="select form-control">
                                                               

                                                               
                                                            </select>
                                                            @error('serve3_price_id')
                                                            <span class="text-danger"> {{$message}}</span>
                                                            @enderror
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-12 col-sm-4 py-2 text-center">
                                                        <div class="form-group">
                                                            ...
                                                            <select  style="height:48px;" name="serve3_tprice_id" id="serve3_tprice_id" class="select form-control">
                                                               

                                                               
                                                            </select>
                                                            @error('serve3_tprice_id')
                                                            <span class="text-danger"> {{$message}}</span>
                                                            @enderror
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-12 col-sm-4 py-2 text-center">
                                                        <div class="form-group">
                                                            ...
                                                            <select  style="height:48px;" name="serve3_total_id" id="serve3_total_id" class="select form-control">
                                                               

                                                               
                                                            </select>
                                                            @error('serve3_total_id')
                                                            <span class="text-danger"> {{$message}}</span>
                                                            @enderror
                                                        </div>
                                                    </div>
...
<script>
        $(function(){
            $(document).on('change','#serve3_id', function(){
                populateServe3_thin();
             return false;
            });
            function  populateServe3_thin(){
                $('option', $('#serve3_thin_id')).remove();
                $('#serve3_thin_id').append($('<option></option>').val('').html());
                var serveId = $('#serve3_id').val() != null ? $('#serve3_id').val() : '{{old('serve3_id')}}';
                $.get("{{Route('gethpric3e')}}",{serve3_id: serveId }, function(data){
               $.each(data, function(val, text){
                   var selectedVal = val == '{{old('serve3_thin_id')}}' ? "selected" : "";
                   $('#serve3_thin_id').append($('<option class="text-center  "'+selectedVal+'></option>').val(val).html(text));
               })
                }, "json");
            }
        });
    ...
    <script>
        $(function(){
            $(document).on('change','#serve3_id', function(){
                populateServe3_price();
             return false;
            });
            function  populateServe3_price(){
                $('option', $('#serve3_price_id')).remove();
                $('#serve3_price_id').append($('<option></option>').val('').html());
                var serveId = $('#serve3_id').val() != null ? $('#serve3_id').val() : '{{old('serve3_id')}}';
                $.get("{{Route('getppric3e')}}",{serve3_id: serveId }, function(data){
               $.each(data, function(val, text){
                   var selectedVal = val == '{{old('serve3_price_id')}}' ? "selected" : "";
                   $('#serve3_price_id').append($('<option '+selectedVal+'></option>').val(val).html(text));
               })
                }, "json");
            }
        });
...
<script>
        $(function(){
            $(document).on('change','#serve3_id', function(){
                populateServe3_tprice();
             return false;
            });
            function  populateServe3_tprice(){
                $('option', $('#serve3_tprice_id')).remove();
                $('#serve3_tprice_id').append($('<option></option>').val('').html());
                var serveId = $('#serve3_id').val() != null ? $('#serve3_id').val() : '{{old('serve3_id')}}';
                $.get("{{Route('gettpric3e')}}",{serve3_id: serveId }, function(data){
               $.each(data, function(val, text){
                   var selectedVal = val == '{{old('serve3_tprice_id')}}' ? "selected" : "";
                   $('#serve3_tprice_id').append($('<option '+selectedVal+'></option>').val(val).html(text));
               })
                }, "json");
            }
        });
...
<script>
        $(function(){
            $(document).on('change','#serve3_id', function(){
                populateServe3_total();
             return false;
            });
            function  populateServe3_total(){
                $('option', $('#serve3_total_id')).remove();
                $('#serve3_total_id').append($('<option></option>').val('').html());
                var serveId = $('#serve3_id').val() != null ? $('#serve3_id').val() : '{{old('serve3_id')}}';
                $.get("{{Route('getopric3e')}}",{serve3_id: serveId }, function(data){
               $.each(data, function(val, text){
                   var selectedVal = val == '{{old('serve3_total_id')}}' ? "selected" : "";
                   $('#serve3_total_id').append($('<option '+selectedVal+'></option>').val(val).html(text));
               })
                }, "json");
            }
        });
