...
class HyperpayServices_in
{
   ...
   private $curl;
    ...
    public function __construct(Client $request_client, happoemint $happoemint)
    {
   ...
   private function buildRequest( $uri, $method,$CustomerCashPayCode,$CurrencyId,$SpId, $data = []){
    ...
    $data = [
      ...
      'Amount'=>$happoemint->hdoctor_serve->price,
    ...
    private function buildcRequest( $uri, $method,$cod, $data = []){
     ...
     'Content-Type: application/json']);
        ...
        if ($responsee->getStatusCode() != 200){
         ...
         $hdoctors = hdoctor::where('id',$happoemint->hdoctor_id)->get();
     ...
     'Content-Type: application/json']);
        ...
        // return $responseee; 
         ...
         return redirect()->route('appoemints',$happoemint->hdoctor_id)->with(['success' => trans('messages.success')]);
     }
