 ...
 <div class="page-banner overlay-dark bg-image" style="background-image: url({{asset('assets/front/u/assets/img/bg_image_1.jpg')}});">
  ...
  @if ($errors->any())
    ...
    <div class="container">
      ...
      <form class="form col-md-12 " action="{{route('fappoemint.save')}}" method="POST"
        ...
        <div class="col-4 col-sm-4   py-2 wow fadeInLeft text-right">
          ...
          <label for="projectinput2"><h3>ألقسم / {{$doctors ->fdepartment-> dept_name}}</h3></label>
            <input type="hidden" name="fdepartment_id" class="form-control" value="{{$doctors ->fdepartment-> id}}" > <h1></h1>
          </div>
          <div class="col-4 col-sm-4   py-2 wow fadeInLeft text-left">
          <label for="projectinput2"><h3>ألتخصص /  {{$doctors ->fspecialty-> special_name}}</h3></label>
            <input type="hidden" name="fspecialty_id" class="form-control" value="{{$doctors ->fspecialty-> id}}" > <h1></h1>
          </div>
          </div>
          <div class="row col-12 ">
          ...
          <label for="projectinput2"><h3>أسم ألطبيب / {{$doctors -> doc_name}}</h3></label> 
            ...
            <input type="hidden" name="fdoctor_id" value="{{$doctors -> id}}" class="form-control" placeholder="Full name" ><h1> </h1>
          </div>
          ...
          <div class="col-12 col-sm-4 py-2 text-center">
            <div class="form-group">
              <label for="projectinput2"><h3>{{trans('servee_trans.Choose serve name')}}</h3>   </label>
                 <select style="height:48px;" name="fserve_id" id="fserve_id" class="select form-control"
                      ...
                      <option>...............</option>
                                                    ...
                                                    <div class="col-12 col-sm-4 py-2 text-center">
                                                        <div class="form-group">
                                                            ...
                                                            <select  style="height:48px;" name="fdoctor_serve_id" id="fdoctor_serve_id" class="select form-control">
                                                               

                                                               
                                                            </select>
                                                            @error('fdoctor_serve_id')
                                                            <span class="text-danger"> {{$message}}</span>
                                                            @enderror
                                                        </div>
                                                    </div>
...
@toastr_render
    ...
    <script>
        $(function(){
            $(document).on('change','#fserve_id', function(){
                populateFdoctor_serves();
             return false;
            });
            function  populateFdoctor_serves(){
                $('option', $('#fdoctor_serve_id')).remove();
                $('#fdoctor_serve_id').append($('<option></option>').val('').html());
                var serveId = $('#fserve_id').val() != null ? $('#fserve_id').val() : '{{old('fserve_id')}}';
                $.get("{{Route('fgetprice')}}",{fserve_id: serveId }, function(data){
               $.each(data, function(val, text){
                   var selectedVal = val == '{{old('fdoctor_serve_id')}}' ? "selected" : "";
                   $('#fdoctor_serve_id').append($('<option '+selectedVal+'></option>').val(val).html(text));
               })
                }, "json");
            }
        });
