                ...
                <div class="content-header-left col-md-6 col-12 mb-2">
                    ...
                    <div class="row breadcrumbs-top">
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{route('fhosbital.dashboard')}}">{{trans('servee_trans.Dashbord')}}</a>
                                </li>
                                        ...
                                        <table
                                            ...
                                            @isset($hdoctor_serves)
                                                @foreach($hdoctor_serves as $hdoctor_serve)
                                                    <tr>
                                                        <td>{{$hdoctor_serve -> fserve -> serv_name}}</td>
                                                        <td>{{$hdoctor_serve -> fdoctor -> doc_name}}</td>
                                                        <td>{{$hdoctor_serve -> price}}</td>
                                                        <td> {{$hdoctor_serve -> getActive()}}</td>
                                                        <td>
                                                            <div class="btn-group" role="group"
                                                                 aria-label="Basic example">
                                                                <a href="{{route('fhosbital.doctorserves.edit',$hdoctor_serve ->id)}}"
                                                                   class="btn btn-outline-primary btn-min-width box-shadow-3 mr-1 mb-1">{{trans('servee_trans.Edite')}}</a>


                                                                <a href="{{route('fhosbital.doctorserves.delete',$hdoctor_serve ->id)}}"
