...
class SikController extends Controller
{
    public function index(){
        ...
        return view('hosbital.sik.index', compact('siks'));
    }
    public function create(){
        $cuontrys = cuontry::get();
        $citys = city::get(); 
        $bloods = blood::get();
        return view('hosbital.sik.create', compact('cuontrys','citys','bloods'));
    }
    public function save(SikRequest $request){
        ...
        return redirect()->route('hosbital.siks')->with(['success'=>'تم ألحفظ  بنجاح']);
    ...
    public function edit($id){
        ...
        if(!$sik){
        return redirect()->route('hosbital.siks')->with(['error'=>'هذا ألمريض غير موجود']);}
        $bloods = blood::active()->get();
        $cuontrys = cuontry::get();
        $citys = city::get(); 
        return view('hosbital.sik.edit',compact('sik','cuontrys','citys','bloods'));
    ...
    public function update($sik_id, SikRequest $request){
        ...
        return redirect()->route('hosbital.siks')->with(['success'=>'تم ألتعديل  بنجاح']);
    ...
    public function destroy($id){
        try{
            $sik = User::find($id);
            if(!$sik){
                return redirect() -> route('hosbital.siks',$id)-> with(['error'=>'هذة ألمريض غير موجودة']);
            }
            $sik -> delete();
            return redirect()->route('hosbital.siks')->with(['success'=>'تم حذف ألمريض بنجاح']);
               }catch(\Exception $ex){
                return redirect()->route('hosbital.siks')->with(['error'=>'هناك خطاء ما يرجى المحاولة فيما بعد']);        
                }    
        ...
        public function showdetail($id){
            ...
            $happoemint = happoemint::where('user_id',$id)->get();
            return view('hosbital.sik.detail',compact('user','happoemint'));
        }
}
