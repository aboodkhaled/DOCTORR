...
class Appoemint3Controller extends Controller
{

    use MeetingZoomTrait;
    public function index(){
        $clinic = clinic::find(auth('clinic')->user()->id);
        $appoemint2s = appoemint3::where('clinic_id',$clinic -> id)->paginate(PAGINATION_COUNT);
        return view('clinic.appoemint3.index', compact('clinic','appoemint2s'));
    }
    public function create(){
       

      $clinic = clinic::find(auth('clinic')->user()->id);
      $serve1s = serve3::where('clinic_id',$clinic -> id)->get();
      $serve1_prices = serve3_price::where('clinic_id',$clinic -> id)->get();
      $serve1_thins = serve3_thin::where('clinic_id',$clinic -> id)->get();
      $users = User::get();
      $serve1_totals = serve3_total::where('clinic_id',$clinic -> id)->get();
      $serve1_tprices = serve3_tprice::where('clinic_id',$clinic -> id)->get();
        return view('clinic.appoemint3.create',compact('clinic','serve1s','serve1_prices','serve1_thins','users','serve1_totals','serve1_tprices'));
    }
    public function save(Request $request){
               ...
               $happoemints->save();
        return redirect()->route('clinic.appoemint3s')->with(['success' => trans('messages.success')]);
    
    }

    public function edit($id){
        try{
        $happoemint = appoemint3::find($id);
        if(!$happoemint){
        return redirect()->route('clinic.appoemint3s')->with(['error'=>'هذا الطبيب غير موجود']);}
      ...
      $hserves = hserve::where('hosbital_id',$hosbital -> id)->get();
        return view('clinic.appoemint3.edit',compact('happoemint','hosbital','hdepartments','hspecialtys','hdoctors','users','hdoctor_serves','hserves'));
        }catch(\Exception $ex){
            return redirect()->route('clinic.appoemint3s')->with(['error'=>'هناك خطاء ما يرجى المحاولة فيما بعد']);   
        }
    ...
    public function update($appoemint_id, Request $request){
        ...
        $happoemint = appoemint3::find($appoemint_id);
        if(!$happoemint)
        return redirect()->route('clinic.appoemint3s')->with(['error'=>'هذا الطبيب غير موجود']);
        DB::beginTransaction();
        ...
        return redirect()->route('clinic.appoemint3s')->with(['success' => trans('messages.Update')]);
    ...
    public function destroy($id){
        try{
            $happoemint = appoemint3::find($id);
            if(!$happoemint){
                return redirect() -> route('clinic.appoemint3s',$id)-> with(['error'=>'هذة ألطبيب غير موجودة']);
            }
            $happoemint -> delete();
            return redirect()->route('clinic.appoemint3s')->with(['success'=>trans('messages.Delete')]);
               }catch(\Exception $ex){
                return redirect()->route('clinic.appoemint3s')->with(['error'=>'هناك خطاء ما يرجى المحاولة فيما بعد']);        
                }    
        ...
        public function showdetail($id){
            $happoemint = appoemint3::where('id',$id)->first();
            $user = hoperation::where('appoemint3_id',$id)->get();
            $hmates = hmate::where('happoemint_id',$id)->get();
            $hdoctors = hdoctor::get();
            return view('clinic.appoemint3.detail',compact('user','happoemint','hmates','hdoctors'));
        }
