...
class VenpharmiceController extends Controller
{
   ...
   public function store(VenpharmiceRequest $request) {
    ...
    if($request->has('photo')){
       ...
       $venpharmices->name = ['en' => $request->name_en, 'ar' => $request->name];
       ...
       $venpharmices->address =['en' => $request->address_en, 'ar' => $request->address];
   ...
   public function update($vp_id,VenpharmiceRequest $request){
    ...
    $request->request->add(['status' => 0]);
          ...
          $venpharmice -> update([ 
            $venpharmice->name = ['en' => $request->name_en, 'ar' => $request->name],
            ...
            $venpharmice->address =['en' => $request->address_en, 'ar' => $request->address],
          ]);
   ...
   public function destroy($v_id){
      ...
      public function print($p_id){
        $venpharmice = venpharmice::orderBy('id')->paginate(PAGINATION_COUNT);
